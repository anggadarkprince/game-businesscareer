		...
		public static function generateTask():Array
		{
			...
			switch (generate)
			{
				case 1:
					...
					task = ["journal", 200, Config.transaction[transaction][2].split("[value]").join("100000"), "task_journal", Config.transaction[transaction][1]]
					break;
				case 2:
					...
					if (list.length != 0)
					{
						...
						task = ["upgrade", 500, "Upgrade asset " + Data.asset[list[asset]].ast_type + " menjadi level " + (int(Data.asset[list[asset]].ast_level) + 1), "task_upgrade", Data.asset[asset].ast_type, int(Data.asset[asset].ast_level) + 1]
					}
				...
				case 3:
					...
					task = ["selling", 200, "Lakukan penjualan dengan total IDR " + target, "task_selling", target];
					break;
				case 4:
					...
					task = ["order", 300, "Siapkan material " + Config.material[product - 1].mtr_item + " sebanyak " + sold + " buah", "task_order", product, sold];
					break;
			}
			return task;
		}
