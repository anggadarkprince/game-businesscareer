		...
		public function generateEvent(generate:int):void {
			...
			if (probability < generate) {
				...
				if(generate > 8){
					...
					totalEvent = (Math.random() < 0.3)?1:0;
				}
		...
		*/
		...
		public function generateCompetitor(shopList:Array, generate:int, map:Map):void
		{
			...
			server.addEventListener(ServerManager.READY, function(event:Event):void
			{
				...
				twoDCoordinate = IsoHelper.get2dFromTileCoordinates(shopList[0].districtCoordinate, map.tileHeight);
				isoPosition = IsoHelper.twoDToIso(twoDCoordinate);
				map.avatarTexture(ServerManager.SERVER_HOST + "assets/images/avatar/" + avatar.player, isoPosition);

				twoDCoordinate = IsoHelper.get2dFromTileCoordinates(shopList[1].districtCoordinate, map.tileHeight);
				isoPosition = IsoHelper.twoDToIso(twoDCoordinate);
				map.avatarTexture(ServerManager.SERVER_HOST + "assets/images/avatar/" + avatar.competitor1, isoPosition);

				twoDCoordinate = IsoHelper.get2dFromTileCoordinates(shopList[2].districtCoordinate, map.tileHeight);
				isoPosition = IsoHelper.twoDToIso(twoDCoordinate);
				map.avatarTexture(ServerManager.SERVER_HOST + "assets/images/avatar/" + avatar.competitor2, isoPosition);
			});
			server.sendRequest();
		 ...
		 */
		public function setShopAttribute1(shop:Shop):void
		{
			shop.shopName = "Player Shop";
			...
			shop.quality = quality;

			// decoration
			var decoration:Object = new Object();
			decoration.modern = 5;
			decoration.colorfull = 6;
			decoration.vintage = 8;
			...
			var productivity:Array = [5, 6, 7, 8, 7, 7, 7];
			shop.productivity = productivity;

			var morale:Array = [5, 7, 5, 8, 8, 9, 7];
			shop.morale = morale;

			var service:Array = [7, 8, 3, 5, 5, 8, 9];
			...
			research.product = 0;
			research.facility = 1;
			shop.businessResearch = research;
			...
			benefit.career = 1;
			benefit.culture = 0;
			benefit.personalization = 0;
			benefit.management = 1;
			benefit.health = 0;
			benefit.education = 0;
			benefit.financial = 1;
			benefit.practice = 1;
			shop.employeeBenefit = benefit;
		 ...
		 * @param shopMarketSetting
		 */
		public function setShopAttribute2(shop:Shop):void
		{
			shop.shopName = "Competitor 1";
		 ...
		 * @param shopMarketSetting
		 */
		public function setShopAttribute3(shop:Shop):void
		{
			shop.shopName = "Competitor 2";
