			...
			if(JSON.parse(server.received.ledger_var) as Array != null){
				...
				{					
					...
					{		
						...
						var row:Array = [
							tmpData[i][j].day,			
							String(tmpData[i][j].description).substr(0,27)+"...",
							(tmpData[i][j].debit!="-")?ValueFormatter.numberFormat(tmpData[i][j].debit):"-",				
							(tmpData[i][j].credit!="-")?ValueFormatter.numberFormat(tmpData[i][j].credit):"-",
							(position=="DEBITS")?ValueFormatter.numberFormat(balance):"-",
							(position=="CREDITS")?ValueFormatter.numberFormat(balance):"-"
						];						
						dataReport.push(row);
					}										
