package sketchproject.objects.loading
{
	...
	public class AssetsLoader extends Sprite
	{
		...
		/**
		 * Default constructor of AssetsLoader
		 */
		public function AssetsLoader()
		{
			...
			assetsTypeText = new TextField(200, 50, "Loading Assets", Assets.getFont("FontSSPBold").fontName, 25, 0x333333);
			assetsTypeText.x = this.width * 0.5 - assetsTypeText.width * 0.5;
			assetsTypeText.y = 420;
			addChild(assetsTypeText);

			assetsDetailText = new TextField(200, 50, "Background Menu Loading", Assets.getFont("FontSSPRegular").fontName, 15, 0x555555);
		...
		{
			TweenMax.to(starbrust, 0.3, {alpha: 1, scaleX: 1, scaleY: 1});

			TweenMax.to(gameLogo, 0.5, {alpha: 1, scaleX: 1, scaleY: 1, ease: Elastic.easeOut, delay: 0.2});

			TweenMax.to(screenCharLeft, 0.5, {y: 250, scaleX: 1, scaleY: 1, ease: Elastic.easeOut, delay: 0.5});

			TweenMax.to(screenCharRight, 0.5, {y: 250, scaleX: 1, scaleY: 1, ease: Elastic.easeOut, delay: 1});

			TweenMax.to(loadingProgress, 0.5, {alpha: 1, scaleX: 1, scaleY: 1, ease: Elastic.easeOut, delay: 0.5});
		...
		public function closeTransition():void
		{
			...
			TweenMax.to(gameLogo, 1, {y: 20, alpha: 0, scaleX: 0.5, scaleY: 0.5, ease: Back.easeIn, delay: 0.8});

			TweenMax.to(loadingProgress, 0.5, {alpha: 0, scaleX: 0.5, scaleY: 0.5, ease: Back.easeIn, delay: 0.3});

			TweenMax.to(starbrust, 0.3, {alpha: 0, scaleX: 0.3, scaleY: 0.3, delay: 1});

			TweenMax.to(hazeContainer, 0.5, {alpha: 0, delay: 0.8});
		}
