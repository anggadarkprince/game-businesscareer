package sketchproject.objects.product.inventory
{
	...
	public class InventoryAssets extends Sprite
	{
		...
		public function InventoryAssets()
		{
			...
			label = new TextField(200, 50, "Depreciation", Assets.getFont(Assets.FONT_SSBOLD).fontName, 18, 0xFFFFFF);
			label.x = 30;
			label.y = 150;
			label.hAlign = HAlign.LEFT;
			addChild(label);
			
			label = new TextField(300, 50, "The decrease in value of assets", Assets.getFont(Assets.FONT_SSREGULAR).fontName, 14, 0xFFFFFF);
			label.x = 30;
			label.y = 170;
			label.hAlign = HAlign.LEFT;
			addChild(label);
			...
			label = new TextField(200, 50, "Useful Life of Asset", Assets.getFont(Assets.FONT_SSBOLD).fontName, 18, 0xFFFFFF);
			label.x = 210;
			label.y = 235;
			label.hAlign = HAlign.LEFT;
			addChild(label);
			
			label = new TextField(150, 50, "Accumulated", Assets.getFont(Assets.FONT_SSBOLD).fontName, 18, 0xFFFFFF);
			label.x = 30;
			label.y = 270;
			label.hAlign = HAlign.LEFT;
			addChild(label);
		...
		private function onRepairTouched(touch:TouchEvent):void
		{
			if(touch.getTouch(buttonRepair, TouchPhase.ENDED))
			{
				if(depreciation <= 0)
				{
					dialogInfo.dialogMessage = "Repair not neccesary";	
				...
				{
					dialogInfo.dialogMessage = "You don't have cash enough";
					Game.overlayStage.swapChildren(dialogInfo, Game.overlayStage.getChildAt(Game.overlayStage.numChildren-1));
					dialogInfo.openDialog();
				}
				else
