package sketchproject.objects.product
{
	...
	public class InventoryFrame extends Sprite
	{
		...
		/**
		 * Default constructor of InventoryFrame.
		 */
		public function InventoryFrame()
		{
			...
			point = new TextField(200, 50, ValueFormatter.numberFormat(Data.point) + " PTS", Assets.getFont(Assets.FONT_CORegular).fontName, 15, 0xFFFFFF);
		...
		/**
		 * Close inventory and posting if neccesary.
		 * 
		 * @param event
		 */
		private function onClosed(event:Event):void
		{
			if (materialFrame.getTotalThrowed() > 0)
			{
				...
				dialog.openDialog();
				materialFrame.resetTotalThrowed();
			}
			closeDialog();
		...
		/**
		 * Set visible and invisible frame.
		 * 
		 * @param frame
		 */
