package sketchproject.objects.business
{
	...
	/**
	 * Screen showed basic information and avatars.
	 * 
	 * @author Angga
	 */
	public class ShopFrame extends Sprite
	{
		...
		/**
		 * Default constructor of ShopFrame.
		 */
		public function ShopFrame()
		{
			...
			label = new TextField(265, 200, "Your goal to produce profit as much as posible and estabilished the competition. Your focus is winning market share from competitor", Assets.getFont(Assets.FONT_SSREGULAR).fontName, 13, 0x333333);
			label.vAlign = VAlign.TOP;
			label.hAlign = HAlign.LEFT;
			label.x = -180;
			label.y = -160;
		...
		/**
		 * Confirm change location of shop.
		 * 
		 * @param event
		 */
		...
		/**
		 * Show confirm dialog if player's select another location.
		 * 
		 * @param event
		 */
		private function onlocationChanged(event:Event):void
		{
			if (!cancelAction)
			{
				...
				confirmDialog.dialogMessage = "Pindah lokasi toko ke " + Config.district[listLocation.selectedIndex][1] + " dengan harga " + Config.district[listLocation.selectedIndex][7] + "?";
				confirmDialog.openDialog();
			}
		}

		/**
		 * Update content of shop information.
		 */
