package sketchproject.objects.dialog
{
	...
	/**
	 * Material detail to show stock of product.
	 * 
	 * @author Angga
	 */
	public class MaterialDetailDialog extends Sprite
	{
		...
		/**
		 * Default constructor of MaterialDetailDialog.
		 * 
		 * @param supplier
		 */
		public function MaterialDetailDialog(supplier:SupplierDialog)
		{
			...
			totalPriceText = new TextField(140, 25, "0", Assets.getFont(Assets.FONT_SSBOLD).fontName, 18, 0x333333);
			...
			infoDialog.addEventListener(DialogBoxEvent.CLOSED, function(event:DialogBoxEvent):void {
		...
		/**
		 * Cancel buy this item, then close the dialog.
		 * 
		 * @param event
		 */
		 ...
		 * check if item exist and expired item same too then merge or update the stock.
		 * 
		 * @param event
		 */
		private function onConfirmed(event:DialogBoxEvent):void
		{
			if (event.result)
			{
				...
				else
				{
					...
					gameObject.material = supply.materialId;
		...
		/**
		 * Get total price from bought items.
		 * 
		 * @return
		 */
		...
		/**
		 * Add total item which is bought.
		 * 
		 * @param event
		 */
		...
		/**
		 * Decrease total item which is bought.
		 * 
		 * @param event
		 */
		...
		/**
		 * Set total price when update total item value.
		 */
		...
		/**
		 * Open confirm dialog or info dialog when player buy item.
		 * 
		 * @param event
		 */
		private function onItemPayed(event:starling.events.Event):void
		{
			if (Data.cash < (supplier.totalInventoryBought + totalPrice))
			{
				...
				infoDialog.dialogMessage = "Kamu tidak punya cukup uang";
				infoDialog.openDialog();
			}
			else if (totalPrice <= 0)
			{
				infoDialog.dialogTitle = "Information";
				infoDialog.dialogMessage = "Membeli minimal 10 item";
				infoDialog.openDialog();
			}
			else
			{
				confirmDialog.dialogMessage = "Membeli item : " + supply.itemName + " sebanyak " + total + " ?";
				confirmDialog.openDialog();
			}
		}

		/**
		 * Set current supply before show the dialog,
		 * called from supplier dialog.
		 * 
		 * @param supply
		 */
		...
		/**
		 * Open / show material detail dialog.
		 */
		...
		/**
		 * Close / hide material detail dialog.
		 */
