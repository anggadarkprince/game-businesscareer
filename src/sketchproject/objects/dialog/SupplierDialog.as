package sketchproject.objects.dialog
{
	...
	/**
	 * Show supplier or workshop dialog.
	 * 
	 * @author Angga
	 */
	public class SupplierDialog extends Sprite
	{
		 ...
		 * 
		 * @param data supplier and list material
		 */
		public function SupplierDialog(data:Object)
		{
			...
			// set type of supplier (market or workshop)
		...
		/**
		 * When close supplier dialog, check if player bought material,
		 * if yes, post the transaction.
		 * 
		 * @param event
		 */
		...
		private function onUpgradeConfirmed(event:DialogBoxEvent):void
		{
			if (event.result)
			{
				if (Data.cash < (totalInventoryBought + upgrade.price))
				{
					...
					dialogInfo.dialogMessage = "Kamu tidak cukup uang untuk upgrade";
				...
				else
				{
					for (var i:int = 0; i < Data.asset.length; i++)
					{
						if (Data.asset[i][2] == upgrade.type)
						{
							...
							gameObject.asset = upgrade.materialId;
							...
							server.addEventListener(ServerManager.READY, function(event:flash.events.Event):void
							{
								...
								var newItem:Array = [upgrade.materialId, upgrade.type, upgrade.itemName, upgrade.texture, upgrade.level];
								Data.asset.splice(i, 1, newItem);
								totalInventoryBought += upgrade.price;

								dialogInfo.dialogTitle = "Asset Upgraded";
								dialogInfo.dialogMessage = "Asset " + upgrade.type + " telah di upgrade ke level " + upgrade.level;
								dialogInfo.openDialog();
		...
		/**
		 * Build market from id of material material item list.
		 * 
		 * @param supply
		 */
		...
		/**
		 * On material item selected, set content and show material detail.
		 * 
		 * @param event
		 */
		...
		/**
		 * Build workshop / list from asset list id.
		 * 
		 * @param supply
		 */
		public function createWorkshop(supply:Array):void
		{
						...
						for (var l:int = 0; l < Data.asset.length; l++)
						{
							...
							if (Data.asset[l].ast_type == Config.asset[i].ast_type && int(Data.asset[l].ast_level) == 3)
							{
								if (isDifferent != Data.asset[l].ast_type)
								{
									...
									totalItem++;
									inventory.push(new Array(1, "-", "-", Data.asset[l].ast_atlas, 0, 3, "-"));
								}
							}
						}
					}
			...
			for (var k:int = startItem; k < endItem; k++)
			{
				var item:UpgradeAdapter = new UpgradeAdapter(k, inventory[k][0], inventory[k][1], inventory[k][2], inventory[k][3], inventory[k][4], inventory[k][5], inventory[k][6]);
		...
		/**
		 * On upgrade selected, the ask for confirmation.
		 * 
		 * @param event
		 */
		private function onUpgradeSelected(event:starling.events.Event):void
		{
			...
			dialogConfirm.dialogMessage = "Upgrade asset " + upgrade.type + " ke level " + upgrade.level + "?";
			dialogConfirm.openDialog();
		}

		/**
		 * Open / show supplier dialog.
		 */
