	...
	public class PostDialog extends Sprite implements IDialog
	{
		...
		public function preparePosting():void
		{
			for(var i:uint = 0; i<Config.transaction.length;i++)
			{
				...
				{					
					this.transaction = String(Config.transaction[i][2]).split("[value]").join(value.toString());
					this.hint = Config.transaction[i][3];
					this.keyDebit = Config.transaction[i][4];
					this.keyCredit = Config.transaction[i][5];
		...
		public function onPrimaryTrigerred(event:starling.events.Event):void
		{
			if(isMatched())
			{
				...
				if(!isTask)
				{
					...
					gameObject.value	 			= this.transactionValue;
		...
		public function onSecondaryTrigerred(event:starling.events.Event):void
		{
			...
			hintDialog.title = this.type.toUpperCase();
			hintDialog.hint = this.hint;
