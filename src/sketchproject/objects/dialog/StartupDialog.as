package sketchproject.objects.dialog
{
	...
	/**
	 * Dialog which show the startup setting.
	 *
	 * @author Angga
	 */
	public class StartupDialog extends DialogOverlay
	{
		...
		/**
		 * Default constructor of StartupDialog.
		 *
		 * @param startup type
		 */
		...
		/**
		 * Set business plan layout.
		 */
		public function layoutBusinessPlan():void
		{
			titleText.text = "Business Plan";

			var businessPlan:TextField = new TextField(500, 220, Data.businessPlan, Assets.getFont(Assets.FONT_SSREGULAR).fontName, 14);
			businessPlan.x = -250;
			businessPlan.y = -115;
			businessPlan.vAlign = VAlign.TOP;
			businessPlan.hAlign = HAlign.CENTER;
			addChild(businessPlan);
		}

		/**
		 * Set personal objective layout.
		 */
		public function layoutPersonalObjective():void
		{
			titleText.text = "Personal Objective";

			var personalObjective:TextField = new TextField(500, 220, Data.personalObjective, Assets.getFont(Assets.FONT_SSREGULAR).fontName, 14);
			personalObjective.x = -250;
			personalObjective.y = -115;
			personalObjective.vAlign = VAlign.TOP;
			personalObjective.hAlign = HAlign.CENTER;
			addChild(personalObjective);
		}

		/**
		 * Set seed financing layout.
		 */
		...
		/**
		 * Override close / hide dialog.
		 */
		override public function closeDialog():void
		{
			if (startupType == STARTUP_PARAMETER)
			{
				server.saveGameData();
			}
