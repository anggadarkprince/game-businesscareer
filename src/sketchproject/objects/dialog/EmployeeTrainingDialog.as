package sketchproject.objects.dialog
{
	...
	/**
	 * Training employee to gain statuses.
	 * 
	 * @author Angga
	 */
	public class EmployeeTrainingDialog extends Sprite
	{
		...
		/**
		 * Default constructor of EmployeeTrainingDialog.
		 */
		public function EmployeeTrainingDialog()
		{
			...
			label = new TextField(170, 50, "Morale", Assets.getFont(Assets.FONT_SSBOLD).fontName, 20, 0x454545);
			...
			label = new TextField(100, 50, "500 K", Assets.getFont(Assets.FONT_SSBOLD).fontName, 20, 0x454545);
			...
			label = new TextField(170, 50, "Productivity", Assets.getFont(Assets.FONT_SSBOLD).fontName, 20, 0x454545);
			label.hAlign = HAlign.LEFT;
			label.vAlign = VAlign.TOP;
			label.x = -113;
			label.y = -40;
			...
			label = new TextField(170, 50, "Services", Assets.getFont(Assets.FONT_SSBOLD).fontName, 20, 0x454545);
			label.hAlign = HAlign.LEFT;
			label.vAlign = VAlign.TOP;
			label.x = -113;
			label.y = 31;
		...
		/**
		 * Get total training cost need to be posted into journal.
		 * 
		 * @return 
		 */
		...
		/**
		 * Confirm and update level of services attribute.
		 * 
		 * @param event
		 */
		private function onServicesTrained(event:Event):void
		{
			...
			dialogConfirm.dialogMessage = "Upgrade status services pegawai?";
		...
		/**
		 * Confirm and update level of productivity.
		 * 
		 * @param event
		 */
		...
		{
			Game.overlayStage.swapChildren(dialogConfirm, Game.overlayStage.getChildAt(Game.overlayStage.numChildren - 1));
			dialogConfirm.dialogMessage = "Upgrade status productivity pegawai?";
			dialogConfirm.removeEventListeners(DialogBoxEvent.CLOSED);
			dialogConfirm.addEventListener(DialogBoxEvent.CLOSED, function(event:DialogBoxEvent):void
			{
				if (event.result)
		...
		/**
		 * Confirm and upgrade level of morale.
		 * 
		 * @param event
		 */
		...
		{
			Game.overlayStage.swapChildren(dialogConfirm, Game.overlayStage.getChildAt(Game.overlayStage.numChildren - 1));
			dialogConfirm.dialogMessage = "Upgrade status morale pegawai?";
			dialogConfirm.removeEventListeners(DialogBoxEvent.CLOSED);
			dialogConfirm.addEventListener(DialogBoxEvent.CLOSED, function(event:DialogBoxEvent):void
			{
				if (event.result)
		...
		/**
		 * Update employee bar level.
		 * 
		 * @param employee current employee data after trained.
		 * @param setNew reset total training
		 */
			...
			TweenMax.to(
				...
				0.5, 
				{
					...
					onComplete: function():void {
						...
						{
							var dialog:PostDialog = new PostDialog("Training", totalTrainingCost);
