package sketchproject.objects.startup
{
	...
	/**
	 * Setting basic configuration such as shop and character profile.
	 *
	 * @author Angga
	 */
	public class StartupConfiguration extends Sprite
	{
		...
		/**
		 * Default constructor of StartupConfiguration.
		 */
		public function StartupConfiguration()
		{
			...
			labelMenu = new TextField(200, 50, "Shop Owner", Assets.getFont(Assets.FONT_ITCERAS).fontName, 18, 0x333333);
			labelMenu.y = 70;
		...
		/**
		 * event handler when logo touched / selected
		...
		private function logoOnTouch(touch:TouchEvent):void
		{
			...
			{
				showTips.showTips();
				showTips.info = "Business " + Image(touch.target).name;
			}
			else
			{
				showTips.hideTips();
		...
		/**
		 * Select avatar of business advisor.
		 *
		 * @param touch
		 */
		...
		/**
		 * Set default logo and advisor selection.
		 */
		...
		/**
		 * Update tooltips position and update player's data by input
		 */
