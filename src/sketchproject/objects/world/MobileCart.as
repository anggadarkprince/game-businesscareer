package sketchproject.objects.world
{
	...
	/**
	 * Simple summaries mobile cart in map, show statistic and profile.
	 * 
	 * @author Angga
	 */
	public class MobileCart extends Sprite
	{
		...
		/**
		 * Default constructor of MobileCart
		 */
		public function MobileCart()
		{
			...
			label = new TextField(150, 25, "MOBILE CART", Assets.getFont(Assets.FONT_SSBOLD).fontName, 12, 0x333333);
			label.pivotX = label.width * 0.5;
		...
		/**
		 * Event handler when this components are touched / hover.
		 * 
		 * @param touch event
		 */
		...
		/**
		 * Start timer to remind player to call the advisor.
		 */
		...
		/**
		 * Stop timer and event to prevent advisor ballon tips show up.
		 */
		...
		/**
		 * Callback function when advisor signal triggered.
		 * 
		 * @param event timer
		 */
		 ...
		 * Shaking the sprite.
		 * 
		 * @param item target sprite
		 * @param repeatCount shaking time
		 */
		...
		/**
		 * Player is making request to show tips from advisor.
		 * 
		 * @param event tips
		 */
		...
		/**
		 * Setup and update mobile cart content.
		 */
