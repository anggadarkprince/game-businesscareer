package sketchproject.objects.world
{
	...
	public class Map extends Sprite
	{
		...
		public function Map(isSimulationStarted:Boolean = true)
		{
			...
			var weather:int = int(Data.weather[0][0]);
			if (weather <= 3)
			{
				if (weather == 1)
				{
		...
		public function addShopLabel(text:String, location:Point, width:int, height:int, visibility:Boolean = true):void
		{
			...
			label.y = IsoHelper.twoDToIso(labelPosition).y - 110;
		...
		public function avatarTexture(texture:String, location:Point):void
		{
			...
			function onComplete(e:Event):void
			{
				...
				var avatarImage:Image = new Image(Assets.getAtlas(Assets.CONTENT, Assets.CONTENT_XML).getTexture("avatar_wrapper"));
				avatarImage.x = location.x + 25;
				avatarImage.y = location.y - 80;
