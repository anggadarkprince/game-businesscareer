package sketchproject.objects.panel
{
	...
	/**
	 * Stats of achievment that earned by player.
	 * 
	 * @author Angga
	 */
	public class AchievementPanel extends Panel
	{
		...
		/**
		 * Default constructor of AchievmentPlanel
		 */
		public function AchievementPanel()
		{
			super(Assets.getAtlas(Assets.CONTENT, Assets.CONTENT_XML).getTexture("icon_star"), Assets.getAtlas(Assets.CONTENT, Assets.CONTENT_XML).getTexture("title_achievement"), "Earn achievements by making good business and life decisions");
		...
		/**
		 * Update number of unlocked achievements.
		 */
		public function updateEarned():void
		{
			for (var i:uint = 0; i < Data.achievement.length; i++)
			{
				var achievement:AchievementAdapter;
				if (i < 5)
				{
					achievement = AchievementAdapter(acheivement1Container.getChildByName(Data.achievement[i].ac_title_atlas));
				}
				else
				{
					achievement = AchievementAdapter(acheivement2Container.getChildByName(Data.achievement[i].ac_title_atlas));
				}
				achievement.earned = Data.achievement[i].earned;
			}
		}
	}
}
