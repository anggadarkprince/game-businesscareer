package sketchproject.states
{
	...
	/**
	 * Populate new data of player when first time playing the game.
	 * 
	 * @author Angga
	 */
	public class Startup extends Sprite implements IState
	{
		...
		/**
		 * Default constructor of Startup.
		 * 
		 * @param game root
		 */
		...
		/**
		 * Event when setup sprite touched.
		 * 
		...
		private function onButtonSetupTouched(touch:TouchEvent):void
		{
			...
			if (touch.getTouch(buttonBusinessProfile, TouchPhase.ENDED))
			{
				...
				setConfigurationDone = true;
				startupPage = 1;
				TweenMax.to(buttonPageActive, 0.5, {x: buttonBusinessProfile.x - 20, ease: Bounce.easeOut});
			...
			if (touch.getTouch(buttonGameParameter, TouchPhase.ENDED))
			{
				...
				startupPage = 2;
				TweenMax.to(buttonPageActive, 0.5, {x: buttonGameParameter.x - 20, ease: Bounce.easeOut});
				Assets.sfxChannel = Assets.sfxClick.play(0, 0, Assets.sfxTransform);
			...
			if (touch.getTouch(buttonPlanObjective, TouchPhase.ENDED))
			{
				...
				setTargetDone = true;
				startupPage = 3;
				TweenMax.to(buttonPageActive, 0.5, {x: buttonPlanObjective.x - 20, ease: Bounce.easeOut});
				Assets.sfxChannel = Assets.sfxClick.play(0, 0, Assets.sfxTransform);
			...
			if (touch.getTouch(buttonSeedFinancial, TouchPhase.ENDED))
			{
				...
				startupPage = 4;
				TweenMax.to(buttonPageActive, 0.5, {x: buttonSeedFinancial.x - 20, ease: Bounce.easeOut});
				Assets.sfxChannel = Assets.sfxClick.play(0, 0, Assets.sfxTransform);
		...
		/**
		 * Data saved and ready to play the game, make sure all default data has been loaded.
		 */
		...
		{
			TweenMax.to(this, 1.2, {y: 0, ease: Bounce.easeOut, delay: 1});
		}
		...
		/**
		 * Update current state, activate play button if all setting has been set.
		 * 
		 * @return void
		 */
		public function update():void
		{
			...
			/** 
			 * everything has been set by default, 
			 * check if all startup is checked then activate play button 
			 **/
