package sketchproject.states
{
	...
	public class Preloading extends Sprite implements IState
	{
		...
		public function initialize():void {	
			...
			readySessionServer = new TransitionScreen(
				TransitionScreen.SERVER_PROGRESS, 
		...
		/**
		 * Game try to check player session for playing the game
		 * @return void
		 */
		private function checkSession():void {
			...
			// handler when server communication complete
			function complete(event:Object):void{				
				...
				if(result=="ready_session")	{
					...
					var noSessionServer:TransitionScreen = new TransitionScreen(
						TransitionScreen.SERVER_EXCLAMATION, 
						"RETRIEVE FAILED", 
			...
			// handle when server communication error
			function error(event:Object):void {
				trace("[ERROR] "+gameServer.error);
				var errorSessionServer:TransitionScreen = new TransitionScreen(
					TransitionScreen.SERVER_EXCLAMATION, 
